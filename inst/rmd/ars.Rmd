...
ars <- function (f, N, x0 = c(-1.0, 1.0), bounds = c(-Inf, Inf), ...) {
  ...
  ## compute h'(x0)
  ##
  ## normalizing h(x0) values gives better accuracy
  ## in finite difference approximation to h'(x)
  avg <- mean(h0)
  if (abs(avg) < dx) {
    avg <- 1.0
  }
  ## Finite difference approximation to h'(x)
  dh0 <- avg*((h(x0 + dx)/avg) - (h0/avg))/dx
  ...
  while (length(x) < N) {
    ...
    if (length(x0) > 0) {
      ...
      ## Evaluate and check h(x0)
      h0 <- h(x0)
      isnum <- is.finite(h0)

      ## Cut to finite values
      x0 <- x0[isnum]
      h0 <- h0[isnum]
